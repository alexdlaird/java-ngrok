...
To create an endpoint, use the [`NgrokClient`](https://javadoc.io/doc/com.github.alexdlaird/java-ngrok/latest/com.github.alexdlaird.ngrok/com/github/alexdlaird/ngrok/NgrokClient.html)'s `connectEndpoint()`
method, which returns an [`Endpoint`](https://javadoc.io/doc/com.github.alexdlaird/java-ngrok/latest/com.github.alexdlaird.ngrok/com/github/alexdlaird/ngrok/protocol/Endpoint.html)
with a reference to the public URL generated by `ngrok`.
...
// Create a pooled Internal Endpoint
final CreateEndpoint createInternalEndpoint = new CreateEndpoint.Builder()
        .withUrl("https://some-endpoint.internal")
        .withUpstream("http://localhost:9000")
        .withPoolingEnabled(true)
        .build();
final Endpoint internalEndpoint = ngrokClient.connectEndpoint(createInternalEndpoint);

// Create an endpoint by name from the config file's `endpoints:` block
final CreateEndpoint createNamedEndpoint = new CreateEndpoint.Builder()
        .withName("my-config-file-endpoint")
        .withUpstream("http://localhost:8000")
        .build();
final Endpoint namedEndpoint = ngrokClient.connectEndpoint(createNamedEndpoint);
```

### Get Active Endpoints

```java
// [<Endpoint: "https://<public_sub>.ngrok.dev" -> "http://localhost:8000">]
...
### Endpoint Configuration in `ngrok`'s Config File

Define endpoints alongside (or in place of) tunnels in `ngrok`'s [config file](https://ngrok.com/docs/agent/config/v3/):

```yaml
version: "3"

endpoints:
  - name: my-config-file-endpoint
    upstream:
...
If `connectEndpoint()` is called without an explicit name and a `java-ngrok-default` definition exists, that
definition is used (the started endpoint will be named `java-ngrok-default-api`).

## Open a Tunnel

> **_Note:_** `ngrok` has [deprecated the tunnels API in favor of endpoints](#create-an-endpoint). `java-ngrok`
> continues to support both, but new integrations should prefer
> [`connectEndpoint()`](#create-an-endpoint).
